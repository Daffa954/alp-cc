<x-app-layout>
    ...
    <div class="space-y-6">
        ...
        <div class="bg-gray-800 rounded-2xl border border-gray-700 overflow-hidden">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 items-center gap-6">
                    ...
                    <div class="flex justify-center">
                        <div class="flex p-1 bg-gray-900/50 rounded-xl border border-gray-700 w-fit">
                            <button id="tabBtnMonthly" class="px-5 py-2 rounded-lg text-sm font-medium transition bg-[#ff6b00] text-white">
                                Bulanan
                            </button>
                            <button id="tabBtnDaily" class="px-5 py-2 rounded-lg text-sm font-medium transition text-gray-400 hover:text-white">
                                Harian
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="flex justify-center md:justify-end">
                        <div id="calendarNav" class="flex items-center space-x-3 bg-gray-700/30 p-1.5 rounded-xl border border-gray-600">
                            <button id="prevMonth" class="p-1.5 text-gray-400 hover:text-white transition bg-gray-800 rounded-lg">
                                <i class="fas fa-chevron-left text-xs"></i>
                            </button>
                            <span id="currentMonthYear" class="text-xs font-bold text-white min-w-[100px] text-center uppercase tracking-wider"></span>
                            <button id="nextMonth" class="p-1.5 text-gray-400 hover:text-white transition bg-gray-800 rounded-lg">
            ...
            <div class="p-6">
                <div id="monthlyView">
                    <div class="grid grid-cols-7 gap-1 mb-2 text-center text-[10px] text-gray-500 font-bold uppercase tracking-widest">
        ...
        <div class="bg-gray-800 rounded-2xl border border-gray-700 p-8">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-5xl mx-auto">
                <a href="{{ route('expenses.create') }}" class="flex flex-col items-center justify-center p-6 border border-gray-700 rounded-2xl hover:border-orange hover:bg-gray-700/50 transition-all group">
                    <div class="w-14 h-14 bg-red-500/10 rounded-2xl flex items-center justify-center mb-4 group-hover:bg-red-500 group-hover:scale-110 transition-all">
                ...
                <a href="{{ route('incomes.create') }}" class="flex flex-col items-center justify-center p-6 border border-gray-700 rounded-2xl hover:border-orange hover:bg-gray-700/50 transition-all group">
                    <div class="w-14 h-14 bg-green-500/10 rounded-2xl flex items-center justify-center mb-4 group-hover:bg-green-500 group-hover:scale-110 transition-all">
                ...
                <a href="{{ route('activities.create') }}" class="flex flex-col items-center justify-center p-6 border border-gray-700 rounded-2xl hover:border-orange hover:bg-gray-700/50 transition-all group">
                    <div class="w-14 h-14 bg-blue-500/10 rounded-2xl flex items-center justify-center mb-4 group-hover:bg-blue-500 group-hover:scale-110 transition-all">
                ...
                <a href="{{ route('reports.index') }}" class="flex flex-col items-center justify-center p-6 border border-gray-700 rounded-2xl hover:border-orange hover:bg-gray-700/50 transition-all group">
                    <div class="w-14 h-14 bg-purple-500/10 rounded-2xl flex items-center justify-center mb-4 group-hover:bg-purple-500 group-hover:scale-110 transition-all">
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                function showMonthly() {
                    ...
                    tabBtnMonthly.className = "px-5 py-2 rounded-lg text-sm font-medium transition bg-[#ff6b00] text-white";
                    tabBtnDaily.className = "px-5 py-2 rounded-lg text-sm font-medium transition text-gray-400 hover:text-white";
                }
                ...
                function showDaily(dateKey) {
                    ...
                    tabBtnDaily.className = "px-5 py-2 rounded-lg text-sm font-medium transition bg-[#ff6b00] text-white";
                    tabBtnMonthly.className = "px-5 py-2 rounded-lg text-sm font-medium transition text-gray-400 hover:text-white";
                    renderDailyList(dateKey);
                }

                function renderDailyList(dateKey) {
                    const dateObj = new Date(dateKey);
                    dailySelectedDate.textContent = `${dateObj.getDate()} ${monthNames[dateObj.getMonth()]} ${dateObj.getFullYear()}`;
                    ...
                    document.getElementById('btnDailyActivity').href = `{{ route('activities.create') }}?date=${dateKey}`;
                    ...
                    if (!data || (!data.expenses?.length && !data.activities?.length && !data.total_income)) {
                        dailyListContainer.innerHTML = `<div class="text-center py-20 text-gray-500 border-2 border-dashed border-gray-700 rounded-3xl mx-auto w-full">Tidak ada transaksi pada tanggal ini.</div>`;
                        return;
                    }
                    ...
                    if (data.activities) {
                        ...
                        data.activities.forEach(act => {
                            html += `<div class="flex items-center p-4 bg-gray-700/50 rounded-2xl border border-gray-600"><i class="fas fa-running text-blue-400 mr-3"></i><span class="text-white text-sm font-medium">${act.title}</span></div>`;
                        });
                        html += '</div>';
                    }
                ...
                function renderCalendar(date) {
                    ...
                    for (let x = firstDayIndex; x > 0; x--) {
                        ...
                        cell.className = 'h-16 w-full rounded-xl flex items-center justify-center text-gray-700 bg-gray-900/10 text-[10px] font-bold';
                    ...
                    for (let i = 1; i <= daysInMonth; i++) {
                        ...
                        cell.className = `min-h-[60px] w-full rounded-xl text-sm flex flex-col items-center justify-start p-1 transition border-2 ${isToday ? 'bg-orange/10 border-[#ff6b00] text-white font-bold' : 'bg-gray-700/20 border-transparent text-gray-400 hover:border-gray-500 hover:bg-gray-700/50'}`;
                        cell.innerHTML = `<span class="text-[10px] mb-1 font-bold">${i}</span>`;

                        if (dayData) {
                            if (dayData.total_income > 0) cell.innerHTML += `<span class="text-[8px] text-green-400 font-bold tracking-tighter">+${(dayData.total_income/1000).toFixed(0)}k</span>`;
                            if (dayData.total_expense > 0) cell.innerHTML += `<span class="text-[8px] text-red-400 font-bold tracking-tighter">-${(dayData.total_expense/1000).toFixed(0)}k</span>`;
                        }
                ...
                tabBtnDaily.onclick = () => showDaily(new Date().toISOString().split('T')[0]);
                    ...
                    scales: {
                        ...
                        x: { 
                            ...
                            ticks: { 
                                color: '#9ca3af', 
                                font: { size: 10 },
                                align: 'center' // Memastikan label tanggal sejajar di tengah sumbu
                            } 
                        }
                    }
                }
    ...
    <style>
        ...
        .stat-card { background: linear-gradient(145deg, #1f2937, #111827); border: 1px solid #374151; transition: all 0.3s ease; }
        .stat-card:hover { border-color: #ff6b00; transform: translateY(-2px); }
    </style>
</x-app-layout>