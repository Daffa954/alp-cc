<x-app-layout>
    ...
    <div class="max-w-2xl mx-auto py-6">
        <div class="bg-gray-800 rounded-2xl border border-gray-700 p-6 shadow-xl relative overflow-hidden">
            ...
            <div
                class="absolute top-0 right-0 -mr-16 -mt-16 w-64 h-64 rounded-full bg-[#ff6b00] opacity-5 blur-3xl pointer-events-none">
            </div>
            ...
            <form method="POST" action="{{ route('expenses.store') }}" class="space-y-6 relative z-10">
                ...
                <div>
                    <label for="description" class="block text-sm font-medium text-gray-300 mb-2">
                        <i class="fas fa-file-alt text-[#ff6b00] mr-2"></i>Deskripsi Pengeluaran
                    </label>
                    <textarea id="description" name="description" rows="3" required
                        ...
                        placeholder="Contoh: Makan siang di warteg...">{{ old('description') }}</textarea>
                    @error('description')
                ...
                <div>
                    ...
                    <div class="relative group">
                        <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                            <span
                                class="text-gray-400 font-semibold group-focus-within:text-[#ff6b00] transition-colors">Rp</span>
                        </div>
                        {{-- PENTING: Gunakan type="text" dan inputmode="numeric" agar format ribuan JS berfungsi --}}
                        <input type="text" inputmode="numeric" id="amount" name="amount" required
                ...
                <div>
                    ...
                    <input type="date" id="date" name="date" required
                        ...
                        value="{{ old('date', request('date', date('Y-m-d'))) }}" style="color-scheme: dark;"
                        {{-- Agar icon kalender bawaan browser jadi putih --}}>
                    @error('date')
                        <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                        ...
                        <select id="activity_id" name="activity_id"
                            ...
                            @foreach ($recentActivities as $activity)
                                <option value="{{ $activity->id }}"
                                    ...
                                    {{ $activity->title }}
                                    ({{ $activity->date_start ? $activity->date_start->format('d M') : '-' }})
                                </option>
                            @endforeach
                        </select>
                        <div
        ...
        <div class="mt-6 bg-gray-800 rounded-2xl border border-gray-700 p-6 shadow-lg">
            ...
            <ul class="space-y-3 text-gray-400 text-sm">
                ...
                <li class="flex items-start">
                    <i class="fas fa-check text-[#ff6b00] mr-3 mt-1"></i>
                    <span>Hubungkan pengeluaran dengan "Aktivitas" jika Anda sedang dalam perjalanan
                        dinas/liburan.</span>
                </li>
            </ul>
        </div>
